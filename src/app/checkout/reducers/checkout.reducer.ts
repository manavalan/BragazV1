...
import { CheckoutState, CheckoutStateRecord } from '../state/checkout.state';
...
export function reducer(state = initialState, { type, payload }: any): CheckoutState {
    ...
    switch (type) {
      ...
      case CheckoutActions.FETCH_CURRENT_ORDER_SUCCESS:
       ...
       _lineItems = payload;
        _lineItemIds = _lineItems.map(lineItem => lineItem.CartId);
        _totalCartItems = payload.length;
       ...
       // _orderState = payload.state;
         ...
         _lineItemEntities = _lineItems.reduce((lineItems: { [CartId: number]: Cart }, lineItem: Cart) => {
          return Object.assign(lineItems, {
            [lineItem.CartId]: lineItem
          });
        }, { });
